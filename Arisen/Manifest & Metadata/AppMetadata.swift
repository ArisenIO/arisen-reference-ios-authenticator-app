...
public struct AppMetadata: Decodable {
    ...
    public var chainParams: ArisenTransaction.ChainParams {
        return ArisenTransaction.ChainParams(chainId: chain.chainId, chainName: chain.chainName, icon: chain.icon)
    }
    ...
    public func chainParams(chainId: String) -> ArisenTransaction.ChainParams? {
        for chain in chains {
