//
//  ArisenTransactionAssert.swift
//
//  Created by Todd Bowden on 10/19/18.
//  Copyright (c) 2017-2019 block.one and its contributors. All rights reserved.
//
//  This Extension will add the arisen.assert::require action to the actions
//
...
extension ArisenTransaction.Action {
    public var isAssertRequire: Bool {
        return account.string == "arisen.assert" && name.string == "require"
    }
}
...
extension ArisenTransaction {
    ...
    struct AssertRequireDataStruct: Encodable {
        ...
        struct ContractAction: Equatable, Encodable {
            var contract: ArisenName
            var action: ArisenName
        }
    ...
    func addAssertRequireAction(appManifest: AppManifest) throws {
        ...
        if let firstAction = actions.first, firstAction.isAssertRequire {
            if areAssertRequireActionsEqual(action1: firstAction, action2: computedAssertRequireAction) {
                ...
                throw ArisenError(.arisenTransactionError, reason: "arisen.assert::require action is invalid")
            }
        }
        // if another action is an assert but is not the first action, throw error
        for action in actions {
            if action.isAssertRequire {
                throw ArisenError(.arisenTransactionError, reason: "arisen.assert::require must the be first action")
            }
        }
        // if the transaction already has signatures, you can't add more actions
        guard signatures == nil || signatures?.count == 0 else {
            throw ArisenError(.arisenTransactionError, reason: "Cannot add arisen.assert::require to transaction with existing signatures")
        }
    ...
    func makeAssertRequireAction(appManifest: AppManifest) throws -> Action {
        ...
        // get arisen.assert abi
        let arisenAssertAbi = try getAbiJsonFile(fileName: "arisen.assert.abi.json")
        
        // get serialization provider
        guard let serializationProvider = serializationProvider else {
            throw ArisenError(.arisenTransactionError, reason: "No serialization provider")
        }
        
        // add manifestId hash
        let manifestJson = try appManifest.toJsonString(convertToSnakeCase: true, prettyPrinted: false)
        let manifestHex = try serializationProvider.serialize(contract: nil, name: "", type: "manifest", json: manifestJson, abi: ArisenAssertAbi)
        ...
        guard let chainParamsJson = String(data: chainParamsJsonData, encoding: .utf8) else {
            throw ArisenError(.arisenTransactionError, reason: "Cannot convert chainParamsJson data to json")
        }
     
        let chainParamsHex = try serializationProvider.serialize(contract: "arisen.assert", name: "", type: "chain_params", json: chainParamsJson, abi: ArisenAssertAbi)
        guard let chainParamsData = Data(hexString: chainParamsHex) else {
            throw ArisenError(.arisenTransactionError, reason: "Cannot convert chainParamsHex to data")
        }
        ...
        let assertRequireAction = try Action(account: "arisen.assert", name: "require", authorization: [], data: assertRequireDataStruct)
        try assertRequireAction.serializeData(abi: arisenAssertAbi, serializationProvider: serializationProvider)
    ...
    private func getAbiJsonFile(fileName: String) throws -> String {
        ...
        guard abiString != "" else {
            throw ArisenError(.serializationProviderError, reason: "Json to hex -- No ABI file found for \(fileName)")
        }
        return abiString
    }
