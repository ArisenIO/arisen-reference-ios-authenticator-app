...
class Validation : NSObject {
    ...
    public static func cttIsValid(transaction: ArisenTransaction) -> CttValidation {
        var isValid = true
        ...
        for action in transaction.actions {
            if let error = action.ricardian?.error {
                if (error != "") {
                    ...
                    if true /* left in for place holder ( UserDefaults.Arisen.bool(forKey: .strictParsingCTT, defaultValue: true) == true ) */{
                        isValid = false
                    }
                }
            }
