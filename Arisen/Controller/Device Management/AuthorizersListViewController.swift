...
class AuthorizersListViewController: UIViewController {
    ...
    //var keychainMetaKeys: [ArisenVault.KeyMetadata]?
    var keys: [ArisenVault.VaultKey]?
    ...
    override func viewDidLoad() {
        ...
        self.showHelperText = !UserDefaults.Arisen.bool(forKey: .autorizersHelperTextHasBeenShown , defaultValue: false)
    ...
    private func deleteKey(publicKey: String, seconds: Int) {
        let timer = Timer.scheduledTimer(withTimeInterval: Double(seconds), repeats: false) { _ in
            //delete key
            do {
                try self.vault.deleteKey(arisenPublicKey: publicKey)
            } catch {
    ...
    private func hideHelperTextRow() {
        self.showHelperText = false; UserDefaults.Arisen.set(true, forKey: .autorizersHelperTextHasBeenShown)
        self.tableView.reloadSections(IndexSet(integer: 0), with: .automatic)
    }
}
...
extension AuthorizersListViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        default:
            ...
            let publicKey = validKey.arisenPublicKey
