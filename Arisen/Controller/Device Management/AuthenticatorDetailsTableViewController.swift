...
class AuthenticatorDetailsTableViewController: UITableViewController {
    ...
    var key: ArisenVault.VaultKey? // set by parent View Controller
    ...
    override func viewDidLoad() {
        ...
        if let validKey = key {
            ...
            self.publicKeyLabel.text = validKey.arisenPublicKey
        }
        deleteKeyButton.isHidden = !canDeleteKey()
        copyKeyButton.isHidden = !UserDefaults.Arisen.bool(forKey: .copyPublicKey, defaultValue: true)
    ...
    @objc func applicationDidBecomeActive() {
        deleteKeyButton.isHidden = !canDeleteKey()
        copyKeyButton.isHidden = !UserDefaults.Arisen.bool(forKey: .copyPublicKey)
    }

    private func canDeleteKey() -> Bool {
        guard let validKey = key else { return false }
        return (UserDefaults.Arisen.bool(forKey: .disableDeleteKeysAbility, defaultValue: true) == false && validKey.isSecureEnclave == false) ? true : false
    }
    ...
    @IBAction func copyKeyPressed(_ sender: UIButton) {
        ...
        UIPasteboard.general.string = validKey.arisenPublicKey
    ...
    @IBAction func deleteKeyPressed(_ sender: UIButton) {
        ...
        guard let validPublicKey = key?.arisenPublicKey else {
