...
import ArisenSwiftAbieosSerializationProvider
import ArisenSwiftVaultSignatureProvider


extension AuthenticatorRequestViewController {
   
    
    func handleTransactionSignatureRequest(payload: ArisenReferenceAuthenticatorSignatureProvider.RequestPayload,
        ...
        do {
            ...
            transaction = try ArisenTransaction.deserialize(serializedTransaction, serializationProvider: ArisenAbieosSerializationProvider())
            ...
            if shouldEnforceActionsWhitelist {
                if let nonWhitelistedActions = transaction.nonWhitelistedActionsList(manifest: manifest) {
                    ...
                    return completion(ArisenTransactionSignatureResponse(error: ArisenError(.signatureProviderError, reason: reason)))
                }
            }
            // add abis
            for abi in transactionSignatureRequest.abis {
                try transaction.abis.addAbi(name: ArisenName(abi.accountName), hex: abi.abi)
            }
            // add assert require
            if shouldAddAssert {
            ...
            // render ricardians
            try transaction.deserializeActionData(exclude: [ArisenName("arisen.assert")])
            do {
                try transaction.renderRicardians(strictParsingCTT: strictParsingCTT)
            } catch (let error) {
                completion(ArisenTransactionSignatureResponse(error: error.arisenError))
            }

        } catch {
            return completion(ArisenTransactionSignatureResponse(error: error.arisenError))
        }
        ...
        // upon success
            ...
            guard possibleError == nil else {
                return completion(ArisenTransactionSignatureResponse(error: ArisenError(.signatureProviderError, reason: possibleError?.reason ?? "")))
            }
    ...
    private func presentConfirmation(transaction: ArisenTransaction, appManifest: AppManifest, reply: @escaping (Bool)->Void) {
    ...
    private func handleConfirmationResponse(transaction: ArisenTransaction, request: ArisenReferenceAuthenticatorSignatureProvider.TransactionSignatureRequest, didAccept: Bool, completion: @escaping (ArisenTransactionSignatureResponse?)->Void) {
        guard didAccept else {
            return completion(ArisenTransactionSignatureResponse(error: ArisenError(.signatureProviderError, reason: "User Declined")))
        }

        transaction.signatureProvider = ArisenVaultSignatureProvider(accessGroup: Constants.vaultAccessGroup, requireBio: true)
        transaction.sign(publicKeys: request.publicKeys) { (result) in
            switch result {
            case .failure(let error):
                return completion(ArisenTransactionSignatureResponse(error: error))
            case .success:
                ...
                guard let signatures = transaction.signatures else {
                    return completion(ArisenTransactionSignatureResponse(error: ArisenError(.signatureProviderError, reason: "No signatures")))
                }
                guard let serializedTransaction = transaction.serializedTransaction else {
                    return completion(ArisenTransactionSignatureResponse(error: ArisenError(.signatureProviderError, reason: "No serialized transaction")))
                }
