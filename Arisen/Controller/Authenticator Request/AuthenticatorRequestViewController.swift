...
class AuthenticatorRequestViewController: UIViewController {
    ...
    private var requestPayload: ArisenReferenceAuthenticatorSignatureProvider.RequestPayload?
    ...
    private lazy var activityIndicator:ActivityIndicatorWithTimeoutMessage = {
        ...
        activityIndicator.doOnCancel { [weak self] in
            ...
            var responsePayload = ArisenReferenceAuthenticatorSignatureProvider.ResponsePayload()
            responsePayload.id = requestPayload.id
            var selectiveDisclosureResponse = ArisenReferenceAuthenticatorSignatureProvider.SelectiveDisclosureResponse()
            selectiveDisclosureResponse.error = ArisenError(ArisenErrorCode.signatureProviderError, reason: "User Declined")
            responsePayload.response.selectiveDisclosure = selectiveDisclosureResponse
            responsePayload.response.transactionSignature?.error = ArisenError(ArisenErrorCode.signatureProviderError, reason: "User Declined")
    ...
    private func parseRequestPayload(){
        ...
        requestPayload = try? jsonDecoder.decode(ArisenReferenceAuthenticatorSignatureProvider.RequestPayload.self, from: payloadData)
        if requestPayload == nil {
            self.showMalformedRequestError(error: AppError(AppErrorCode.malformedRequestError, reason: "Request payload JSON does not conform to ArisenMobileAuthenticatorSignatureProvider.RequestPayload"))
            return
        }
    }
        ...
        appManifestProvider.getAppManifest(payload: requestPayload, requireChainId: requireChainId, requireHashIntegrity: true, completion: { [weak self] (manifest, manifestError) in
            ...
            guard let manifest = manifest else {
                ...
                if manifestError.isReturnable {
                    let returnableError = ArisenError(.signatureProviderError, reason: manifestError.reason)
                    var responsePayload = ArisenReferenceAuthenticatorSignatureProvider.ResponsePayload()
                    responsePayload.id = requestPayload.id
                    if requestPayload.request.selectiveDisclosure != nil {
                        responsePayload.response.selectiveDisclosure = ArisenReferenceAuthenticatorSignatureProvider.SelectiveDisclosureResponse(error: returnableError)
                    }
                    if requestPayload.request.transactionSignature != nil {
                        responsePayload.response.transactionSignature = ArisenReferenceAuthenticatorSignatureProvider.TransactionSignatureResponse(error: returnableError)
                    }
            ...
            strongSelf.handleSelectiveDisclosureRequest(payload: requestPayload, manifest: manifest, completion: { (selectiveDisclosureResponse) in
                ...
                strongSelf.handleTransactionSignatureRequest(payload: requestPayload, manifest: manifest, completion: { (transactionSignatureResponse) in
                    ...
                    if let transactionSignatureResponse = transactionSignatureResponse {
                        responsePayload.response.transactionSignature = ArisenReferenceAuthenticatorSignatureProvider.TransactionSignatureResponse(arisenTransactionSignatureResponse: transactionSignatureResponse)
                    }
	...
	func sendResponse(requestPayload: ArisenReferenceAuthenticatorSignatureProvider.RequestPayload,
                  responsePayload: ArisenReferenceAuthenticatorSignatureProvider.ResponsePayload) {
    ...
    func getActiveSecurityExclusions() -> SecurityExclusions? {
        ...
        if  UserDefaults.Arisen.bool(forKey:.insecureMode) == true {
