# ARISEN Reference iOS Authenticator App
![ARISEN Labs](https://img.shields.io/badge/ARISEN-Labs-5cb3ff.svg)

An iOS app demonstrating inter-application signing of transactions using various ARISEN Labs specifications and technologies. Transactions can be proposed from:
* web applications running in Mobile Safari
* other native iOS apps using [ARISEN SDK for Swift](https://github.com/ARISEN/arisen-swift)
...
* [Configuring Apps to Work with ARISEN Reference iOS Authenticator App](#configuring-apps-to-work-with-ARISEN-reference-ios-authenticator-app)
  * [Prerequisites](#prerequisites)
  * [Integrating Native iOS Apps Using ARISEN SDK for Swift](#integrating-native-ios-apps-using-ARISEN-sdk-for-swift)
...
This reference implementation is one of two examples (see also the [ARISEN Reference Chrome Extension Authenticator App
](https://github.com/ARISEN/arisen-reference-chrome-extension-authenticator-app)) for wallet developers and presents possible solutions to the problems above using a native iOS Authenticator application. The ARISEN Reference iOS Authenticator App provides the following features, as examples:
...
* It displays richly-formatted [Ricardian Contracts](https://github.com/ARISEN/ricardian-spec), which provide users with a human-readable explanation of the action(s) the app is proposing and allows users to approve or reject the terms of the contract(s).
* By following the [Manifest Specification](https://github.com/ARISEN/manifest-spec), the Reference Authenticator App displays metadata about the requesting application to end users any time they are prompted to trust an app or sign a transaction from one. This provides users with an improved sense of trust in the requesting application and the signing ceremony itself. The Reference Authenticator App also runs various transaction pre-flight security checks, comparing the contents of a transaction request with what integrating apps have declared about themselves.

### Definitions
* _Reference App_, _Authenticator App_, _Reference iOS Authenticator App_: This app, the ARISEN Reference iOS Authenticator App.
* _Integrating App_, _Requesting App_: Native iOS apps or web applications that request public keys or the signing of transactions from this ARISEN Reference iOS Authenticator App.
...
* [ARISEN Authentication Transport Protocol Spec](https://github.com/ARISEN/arisen-authentication-transport-protocol-spec)
...
1. Change the bundle identifier from `one.block.arisen` to a new bundle identifer of your choice.
...
* **Add a new Authenticator** (key) to the app using the "Add Additional Authenticators" button at the bottom of the Auths screen. (This should be a private key already on your ARISEN account.)
* **Use the newly-generated Secure key**. If you go this route, you will need to enable "Show Copy Public Key Option" in iOS Settings > Authenticator (see screenshot below), copy the key, and use another tool, such as [Arisecli](https://developers.eos.io/arisen-arisecli/reference), to add the key to your blockchain account.
...
If you want to start out by test driving the ARISEN Reference iOS Authenticator App for yourself, we recommend checking out our [Tropical Example web app](https://github.com/ARISEN/tropical-example-web-app/). Tropical Example is a mock web application for renting properties and provides instructions and a script for setting up a local chain bootstrapped with all of the necessary, compliant contracts for making the experience work.

Specifically, follow the instructions under the [Running Tropical Example](https://github.com/ARISEN/tropical-example-web-app/#running-tropical-example) header. (Of course, we recommend reading the rest of the README there too, which will provide more context around how the pieces work together to provide the user with a secure and positive user experience.)

One **important note** to keep in mind: the web application and local blockchain will both be running on `localhost`, so you will need to use the ARISEN Reference iOS Authenticator App in the [Simulator](#simulator) running on the same machine for this particular example web app.
...
A [Selective Disclosure Request](https://github.com/ARISEN/internal-arisen-authentication-transport-protocol-spec#selective-disclosure-request) is akin to a signature provider's `getAvailableKeys()` call. In fact, in most cases, that's exactly what is happening under the hood.
...
3. iOS will switch you over to the ARISEN Reference iOS Authenticator App where you will be presented with the Selective Disclosure request. The information from this selective disclosure comes from the [metadata associated with the application manifest](https://github.com/ARISEN/manifest-spec). You can either Allow or Deny this request.
...
## Configuring Apps to Work with ARISEN Reference iOS Authenticator App
The Reference Authenticator Apps follow the [ARISEN Auth Transport Protocol](https://github.com/ARISEN/arisen-authentication-transport-protocol-spec) specification defining inter-application transports and the payloads sent over those transports.
...
### Prerequisites
* Your app must conform to the [Manifest Specification](https://github.com/ARISEN/manifest-spec).
* Contracts for any action included in a transaction by your app must include a Ricardian contract conforming with the [Ricardian Specification](https://github.com/ARISEN/ricardian-spec).
* Chains on which you wish to transact must have the [`arisen.assert` contract](#assert-contract) deployed and a chain name and icon declared.
* Apps must be served over HTTPS or locally from `localhost`.

### Integrating Native iOS Apps Using ARISEN SDK for Swift
Configure the [ARISEN SDK for Swift](https://github.com/ARISEN/arisen-swift) `ArisenTransaction` with the [Reference iOS Authenticator Signature Provider](https://github.com/ARISEN/arisen-swift-reference-ios-authenticator-signature-provider).

See the [Basic Usage](https://github.com/ARISEN/arisen-swift#basic-usage) documentation in the ARISEN SDK for Swift repo for instructions on configuring ARISEN SDK for Swift with a signature provider.

Additionally, refer to [Reference iOS Authenticator Signature Provider](https://github.com/ARISEN/arisen-swift-reference-ios-authenticator-signature-provider) README for more information around using that signature provider to interact with the ARISEN Reference iOS Authenticator App.

### Integrating Web Apps from Mobile Safari
Choose a solution for interacting with the Reference iOS Authenticator App:

1. Use the [Universal Authenticator Library](https://github.com/ARISEN/universal-authenticator-library) and the [UAL for ARISEN Reference Authenticator](https://github.com/ARISEN/ual-ARISEN-reference-authenticator) (_easiest_).
    ...
    * Read the documentation on the [Tropical example app](https://github.com/ARISEN/tropical-example-web-app) to learn how to use the UAL.
1. Directly instantiate [EOSJS](https://github.com/ARISEN/arisenjs) with the [EOSJS Signature Provider for ARISEN Authenticator in iOS](https://github.com/ARISEN/eosjs-ios-browser-signature-provider-interface).
...
1. Directly use the iOS Deep Linking API.
    * The integrating app will need to post requests and listen for responses following the [ARISEN Auth Transport Protocols](https://github.com/ARISEN/arisen-authentication-transport-protocol-spec) envelope formats:
      * Requests: [URL Query String Payload](https://github.com/ARISEN/arisen-authentication-transport-protocol-spec#url-query-string-payload) transport
      * Responses: [URL Hash Fragment Identifier](https://github.com/ARISEN/arisen-authentication-transport-protocol-spec#url-hash-fragment-identifier) transport
...
Under the hood, integrating apps interact with the ARISEN Reference iOS Authenticator App through the [iOS Deep Link](https://developer.apple.com/documentation/uikit/core_app/allowing_apps_and_websites_to_link_to_your_content/defining_a_custom_url_scheme_for_your_app) transport and the [ARISEN Authentication Transport Protocol](https://github.com/ARISEN/internal-ARISEN-authentication-transport-protocol-spec). However, that communication is generally abstracted away from app developers by the appropriate signature providers:
* [ARISEN SDK for Swift: Reference iOS Authenticator Signature Provider](https://github.com/ARISEN/arisen-swift-reference-ios-authenticator-signature-provider) for native iOS apps
* [EOSJS Signature Provider for ARISEN Authenticator in iOS](https://github.com/ARISEN/eosjs-ios-browser-signature-provider-interface) for web apps using [EOSJS](https://github.com/ARISEN/eosjs)

Web applications using the [Universal Authenticator Library](https://github.com/ARISEN/universal-authenticator-library) will find it easiest to use the [UAL for ARISEN Reference Authenticator](https://github.com/ARISEN/ual-ARISEN-reference-authenticator), which allows web apps to work with both this app on iOS and [its Chrome Extension counterpart](https://github.com/ARISEN/ARISEN-reference-chrome-extension-authenticator-app).

Internally, the iOS Authenticator App uses [ARISEN SDK for Swift](https://github.com/ARISEN/ARISEN-swift) along with the [ARISEN SDK for Swift: Vault Signature Provider](https://github.com/ARISEN/arisen-swift-vault-signature-provider) to get available keys from and sign with the device's Secure Enclave or the Keychain.

While the flowchart above emphasizes the _request_ lifecycle, simply reverse the arrows and you have the _response_ path. The transaction is signed in the Secure Enclave and/or Keychain, sent back to the Reference iOS Authenticator App via ARISEN SDK for Swift, and back through the iOS Deep Link Transport via the callback URL specified in the original request, eventually reaching all the way back to requesting app and user.

### Restricting Reference iOS Authenticator App Communication with Chains/Nodes
The ARISEN Reference iOS Authenticator App _does not directly communicate with any ARISEN chains or nodes_. It is entirely chain unaware and relatively chain agnostic. In fact, there is no way to configure networks or RPC endpoints in the iOS app because it doesn't need them.
...
The ARISEN Reference iOS Authenticator App adheres to the [Manifest Specification](https://github.com/ARISEN/manifest-spec). All [checks](https://github.com/ARISEN/manifest-spec#how-it-works) described in the Manifest Specification are validated by the iOS app.

### Assert Contract
As part of the Manifest Specification, the ARISEN Reference iOS Authenticator App will append an invisible-to-the-user [`ARISEN.assert::require` action](https://github.com/ARISEN/ARISEN-reference-chrome-extension-authenticator-app/blob/master/src/contracts/ARISEN.assert.abi.json#L52) with the following parameters:
   * `chain_params_hash`: Hash of the chain information from the [Application Metadata](https://github.com/ARISEN/manifest-spec#application-metadata-specification) for the chain defined in the request envelope.
   * `manifest_id`: Hash of the [Manifest](https://github.com/ARISEN/manifest-spec#application-manifest-spec) for the chain defined in the request envelope.
...
This has another implication: block producers must deploy the `ARISEN.assert` contract and declare a chain name and chain icon.
...
### Security
The ARISEN Reference iOS Authenticator App uses the platform-supplied Secure Enclave and Keychain to secure all private keys. Private keys stored in the Secure Enclave cannot even be known by the iOS Authenticator App itself.
...
The integrating application must add the [securityExclusions](https://github.com/ARISEN/ARISEN-authentication-transport-protocol-spec#securityexclusions-optional) options to the [Request Envelope](https://github.com/ARISEN/ARISEN-authentication-transport-protocol-spec#request-envelope) as part of the [ARISEN Transport Protocol](https://github.com/ARISEN/ARISEN-authentication-transport-protocol-spec#securityexclusions-optional).

If using the [UAL for ARISEN Reference Authenticator](https://github.com/ARISEN/ual-ARISEN-reference-authenticator), these can be [passed in along with the authenticator options during UAL instantiation](https://github.com/ARISEN/ual-ARISEN-reference-authenticator#supported-environments). The same applies when using the [EOSJS Signature Provider for ARISEN Authenticator in iOS](https://github.com/ARISEN/eosjs-ios-browser-signature-provider-interface) directly.
...
* Cross-platform signing. Could the [ARISEN Reference Chrome Extension Authenticator App](https://github.com/ARISEN/arisen-reference-chrome-extension-authenticator-app) on desktop trigger a push to sign from this iOS app?
